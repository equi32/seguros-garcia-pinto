...
jQuery($=> {
    ...
    $("#submit_btn1 , #submit_btn").on('click', function () {
        ...
        let urlSend = $(this).closest('.getin_form').attr('action');
        let result;
        ...
        if (proceed) {
            ...
            postData = $(this).closest('.getin_form').serialize();
            //Muestro la espera
            waitingDialog.show('Enviando...');
            //Ajax post data to server
            $.post(urlSend, postData, function (response) {
                //Una vez que respondió, oculto
                waitingDialog.hide();
    ...
    //Cotización
    $("#submit_btn_cotization").on('click', function () {
        
        let urlSend = $('#form-cotization').attr('action');
        let result = $('#result_cotization');
        //simple validation at client's end
        let postData, output;
        let proceed = true;
        //everything looks good! proceed...
        if (proceed) {
            ...
            postData = $('#form-cotization').serialize();
            //Muestro la espera
            waitingDialog.show('Enviando...');
            //Ajax post data to server
            $.post(urlSend, postData, function (response) {
                ...
                if (response.type === 'error') {
                    ...
                    output = '<div class="alert-success" style="padding:10px; margin-bottom:25px;">' + response.text + '</div>';
                    //reset values in all input fields
                    $('.getin_form input:not([name="_token"])').val('');
                    $('.getin_form textarea').val('');
    ...
    $('.Pricing-toggle-button').on('click', function () {
        ...
        //Nuevo
        if (!$(this).hasClass('active')) {
            //Tengo que tomar la clase
            if($(this).hasClass('vehiculos')){
                //Muestro vehiculos
                $("#cotization-content").find('.accidentes').hide();
                $("#cotization-content").find('.hogar').hide();
                $("#cotization-content").find('.vehiculos').show();
                $('input[name="type_cotization"]').val('vehiculos');
            }
            if($(this).hasClass('hogar')){
                //Muestro hogar
                $("#cotization-content").find('.accidentes').hide();
                $("#cotization-content").find('.vehiculos').hide();
                $("#cotization-content").find('.hogar').show();
                $('input[name="type_cotization"]').val('hogar');
            }
            if($(this).hasClass('accidentes')){
                //Muestro hogar
                $("#cotization-content").find('.hogar').hide();
                $("#cotization-content").find('.vehiculos').hide();
                $("#cotization-content").find('.accidentes').show();
                $('input[name="type_cotization"]').val('accidentes');
            }
            $(this).addClass('active').siblings().removeClass('active');
        }
    });
